...
def prepare_inputs(
    bs, idim, odim, maxin_len, maxout_len, spk_embed_dim=None, spc_dim=None, device=torch.device("cpu"),
):
...
def test_tacotron2_trainable_and_decodable(model_dict):
    ...
    batch = prepare_inputs(bs, idim, odim, maxin_len, maxout_len, model_args["spk_embed_dim"], model_args["spc_dim"],)
...
def test_tacotron2_gpu_trainable_and_decodable(model_dict):
    ...
    batch = prepare_inputs(
        bs, idim, odim, maxin_len, maxout_len, model_args["spk_embed_dim"], model_args["spc_dim"], device=device,
    )
