...
class LoadInputsAndTargets(object):
    ...
    def __init__(
        ...
        if (use_second_target or use_speaker_embedding) and mode != "tts":
            logging.warning('"use_second_target" and "use_speaker_embedding" is ' "used only for tts mode")
    ...
    def __call__(self, batch):
        ...
        for uttid, info in batch:
            ...
            if self.load_output:
                if self.mode == "mt":
                    x = np.fromiter(map(int, info["output"][1]["tokenid"].split()), dtype=np.int64)
        ...
        if self.mode == "asr":
            return_batch, uttid_list = self._create_batch_asr(x_feats_dict, y_feats_dict, uttid_list)
        elif self.mode == "tts":
            _, info = batch[0]
            eos = int(info["output"][0]["shape"][1]) - 1
            return_batch, uttid_list = self._create_batch_tts(x_feats_dict, y_feats_dict, uttid_list, eos)
        elif self.mode == "mt":
            return_batch, uttid_list = self._create_batch_mt(x_feats_dict, y_feats_dict, uttid_list)
        else:
    ...
    def _create_batch_asr(self, x_feats_dict, y_feats_dict, uttid_list):
        ...
        if len(nonzero_sorted_idx) != len(xs[0]):
            logging.warning(
                "Target sequences include empty tokenid (batch {} -> {}).".format(len(xs[0]), len(nonzero_sorted_idx))
            )
    ...
    def _create_batch_mt(self, x_feats_dict, y_feats_dict, uttid_list):
        ...
        if len(nonzero_sorted_idx) != len(xs):
            logging.warning(
                "Target sequences include empty tokenid (batch {} -> {}).".format(len(xs), len(nonzero_sorted_idx))
            )
    ...
    def _create_batch_tts(self, x_feats_dict, y_feats_dict, uttid_list, eos):
        ...
        if self.load_input:
            ...
            return_batch = OrderedDict([(x_name, xs), (y_name, ys), (spembs_name, spembs), (spcs_name, spcs)])
