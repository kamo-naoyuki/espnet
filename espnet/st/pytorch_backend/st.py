...
class CustomConverter(ASRCustomConverter):
    ...
    def __call__(self, batch, device=torch.device("cpu")):
        ...
        if self.asr_task:
            ys_pad_asr = pad_list([torch.from_numpy(np.array(y[1])).long() for y in ys_asr], self.ignore_id,).to(device)
        else:
            ys_pad_asr = None
...
def train(args):
    ...
    # Setup a converter
    converter = CustomConverter(subsampling_factor=subsampling_factor, dtype=dtype, asr_task=args.asr_weight > 0)
    ...
    trainer.extend(
        extensions.PlotReport(
            ["main/acc", "validation/main/acc", "main/acc_asr", "validation/main/acc_asr"],
            "epoch",
            file_name="acc.png",
        )
    )
    trainer.extend(extensions.PlotReport(["main/bleu", "validation/main/bleu"], "epoch", file_name="bleu.png"))
