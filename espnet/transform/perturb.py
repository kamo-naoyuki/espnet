...
class SpeedPerturbation(object):
    ...
    def __init__(
        self, lower=0.9, upper=1.1, utt2ratio=None, keep_length=True, res_type="kaiser_best", seed=None,
    ):
    ...
    def __repr__(self):
        if self.utt2ratio is None:
            return "{}(lower={}, upper={}, " "keep_length={}, res_type={})".format(
                self.__class__.__name__, self.lower, self.upper, self.keep_length, self.res_type,
            )
        else:
            return "{}({}, res_type={})".format(self.__class__.__name__, self.utt2ratio_file, self.res_type)
    ...
    def __call__(self, x, uttid=None, train=True):
        ...
        if self.keep_length:
            ...
            if len(y) > len(x):
                ...
                pad_width = [(diff // 2, (diff + 1) // 2)] + [(0, 0) for _ in range(y.ndim - 1)]
                y = numpy.pad(y, pad_width=pad_width, constant_values=0, mode="constant")
        return y
...
class BandpassPerturbation(object):
    ...
    def __call__(self, x_stft, uttid=None, train=True):
        ...
        if x_stft.ndim == 1:
            raise RuntimeError("Input in time-freq domain: " "(Time, Channel, Freq) or (Time, Freq)")
...
class VolumePerturbation(object):
    ...
    def __repr__(self):
        if self.utt2ratio is None:
            ...
            return '{}("{}", dbunit={})'.format(self.__class__.__name__, self.utt2ratio_file, self.dbunit)
...
class NoiseInjection(object):
    ...
    def __init__(
        self, utt2noise=None, lower=-20, upper=-5, utt2ratio=None, filetype="list", dbunit=True, seed=None,
    ):
        ...
        if utt2noise is not None and utt2ratio is not None:
            if set(self.utt2ratio) != set(self.utt2noise):
                raise RuntimeError("The uttids mismatch between {} and {}".format(utt2ratio, utt2noise))
...
class RIRConvolve(object):
    ...
    def __call__(self, x, uttid=None, train=True):
        ...
        if x.ndim != 1:
            ...
            raise RuntimeError("Input x must be one dimensional array, but got {}".format(x.shape))

        rir, rate = self.utt2rir[uttid]
        if rir.ndim == 2:
            # FIXME(kamo): Use chainer.convolution_1d?
            # return [Time, Channel]
            return numpy.stack([scipy.convolve(x, r, mode="same") for r in rir], axis=-1)
        else:
