...
class ClassifierWithState(link.Chain):
    ...
    def __init__(
        self, predictor, lossfun=softmax_cross_entropy.softmax_cross_entropy, label_key=-1,
    ):
...
class RNNLM(chainer.Chain):
    ...
    def __init__(self, n_vocab, n_layers, n_units, typ="lstm"):
        ...
        with self.init_scope():
            ...
            self.rnn = (
                chainer.ChainList(*[L.StatelessLSTM(n_units, n_units) for _ in range(n_layers)])
                if typ == "lstm"
        ...
        if self.typ == "lstm":
            ...
            if state["h"][0] is None:
                ...
                with chainer.backends.cuda.get_device_from_id(self._device_id):
                    state["h"][0] = chainer.Variable(xp.zeros((emb.shape[0], self.n_units), dtype=emb.dtype))
...
class BPTTUpdater(training.updaters.StandardUpdater):
    ...
    def update_core(self):
        ...
        for i in six.moves.range(sequence_length):
            ...
            state, loss_batch = optimizer.target(state, chainer.Variable(x[:, i]), chainer.Variable(t[:, i]))
...
def train(args):
    ...
    train_iter = ParallelSentenceIterator(
        train, args.batchsize, max_length=args.maxlen, sos=eos, eos=eos, shuffle=not use_sortagrad,
    )
    val_iter = ParallelSentenceIterator(val, args.batchsize, max_length=args.maxlen, sos=eos, eos=eos, repeat=False)
    ...
    trainer.extend(
        extensions.LogReport(postprocess=compute_perplexity, trigger=(args.report_interval_iters, "iteration"),)
    )
    trainer.extend(
        extensions.PrintReport(["epoch", "iteration", "perplexity", "val_perplexity", "elapsed_time"]),
    ...
    if use_sortagrad:
        trainer.extend(
            ShufflingEnabler([train_iter]), trigger=(args.sortagrad if args.sortagrad != -1 else args.epoch, "epoch"),
        )
