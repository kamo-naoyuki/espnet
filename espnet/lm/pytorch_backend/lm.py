...
def train(args):
    ...
    train_iter = ParallelSentenceIterator(
        train, batch_size, max_length=args.maxlen, sos=eos, eos=eos, shuffle=not use_sortagrad,
    )
    val_iter = ParallelSentenceIterator(val, batch_size, max_length=args.maxlen, sos=eos, eos=eos, repeat=False)
    ...
    updater = BPTTUpdater(train_iter, model, optimizer, gpu_id, gradclip=args.gradclip, use_apex=use_apex)
    ...
    trainer.extend(
        extensions.PrintReport(["epoch", "iteration", "main/loss", "perplexity", "val_perplexity", "elapsed_time"]),
    ...
    if args.test_label:
        ...
        test_iter = ParallelSentenceIterator(test, batch_size, max_length=args.maxlen, sos=eos, eos=eos, repeat=False)
