...
class Reporter(chainer.Chain):
    ...
    def report(
        self, loss_asr, loss_mt, loss_st, acc_asr, acc_mt, acc, cer_ctc, cer, wer, bleu, mtl_loss,
    ):
...
class E2E(STInterface, torch.nn.Module):
    ...
    def __init__(self, idim, odim, args):
        ...
        if self.mt_weight > 0:
            ...
            self.enc_mt = encoder_for(args, args.eunits, subsample=np.ones(args.elayers + 1, dtype=np.int))
    ...
    def forward(self, xs_pad, ilens, ys_pad, ys_pad_src):
        ...
        if self.training or (self.asr_weight == 0 or self.mtlalpha == 1 or not (self.report_cer or self.report_wer)):
            ...
            nbest_hyps_asr = self.dec_asr.recognize_beam_batch(
                hs_pad, torch.tensor(hlens), lpz, self.recog_args, self.char_list, self.rnnlm,
            )
            # remove <sos> and <eos>
