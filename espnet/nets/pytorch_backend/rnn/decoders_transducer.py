...
class DecoderRNNT(torch.nn.Module):
    ...
    def recognize_beam(self, h, recog_args, rnnlm=None):
        ...
        if rnnlm:
            kept_hyps = [{"score": 0.0, "yseq": [self.blank], "z_prev": z_list, "c_prev": c_list, "lm_state": None}]
        else:
        ...
        for i, hi in enumerate(h):
            ...
            while True:
                ...
                y, (z_list, c_list) = self.rnn_forward(ey[0], (new_hyp["z_prev"], new_hyp["c_prev"]))
        ...
        if normscore:
            nbest_hyps = sorted(kept_hyps, key=lambda x: x["score"] / len(x["yseq"]), reverse=True)[:nbest]
        else:
...
class DecoderRNNTAtt(torch.nn.Module):
    ...
    def recognize(self, h, recog_args):
        ...
        att_c, att_w = self.att[0](h.unsqueeze(0), [h.size(0)], self.dropout_dec[0](z_list[0]), None)
        ...
        for hi in h:
            ...
            if pred != self.blank:
                ...
                att_c, att_w = self.att[0](h.unsqueeze(0), [h.size(0)], self.dropout_dec[0](z_list[0]), att_w)
    ...
    def recognize_beam(self, h, recog_args, rnnlm=None):
        ...
        if rnnlm:
            ...
            kept_hyps = [{"score": 0.0, "yseq": [self.blank], "z_prev": z_list, "c_prev": c_list, "a_prev": None}]
        ...
        for i, hi in enumerate(h):
            ...
            while True:
                ...
                att_c, att_w = self.att[0](
                    h.unsqueeze(0), [h.size(0)], self.dropout_dec[0](new_hyp["z_prev"][0]), new_hyp["a_prev"],
                )
