...
class RNNP(torch.nn.Module):
    ...
    def __init__(self, idim, elayers, cdim, hdim, subsample, dropout, typ="blstm"):
        ...
        for i in six.moves.range(elayers):
            if i == 0:
                ...
                inputdim = hdim
            rnn = (
                torch.nn.LSTM(inputdim, cdim, dropout=dropout, num_layers=1, bidirectional=bidir, batch_first=True,)
                if "lstm" in typ
    ...
    def forward(self, xs_pad, ilens, prev_state=None):
        ...
        for layer in six.moves.range(self.elayers):
            ...
            projected = getattr(self, "bt" + str(layer))(ys_pad.contiguous().view(-1, ys_pad.size(2)))
...
class RNN(torch.nn.Module):
    ...
    def __init__(self, idim, elayers, cdim, hdim, dropout, typ="blstm"):
        ...
        bidir = typ[0] == "b"
        self.nbrnn = (
            torch.nn.LSTM(idim, cdim, elayers, batch_first=True, dropout=dropout, bidirectional=bidir,)
            if "lstm" in typ
            else torch.nn.GRU(idim, cdim, elayers, batch_first=True, dropout=dropout, bidirectional=bidir,)
        )
        if bidir:
...
class VGG2L(torch.nn.Module):
    ...
    def forward(self, xs_pad, ilens, **kwargs):
        ...
        xs_pad = xs_pad.view(
            xs_pad.size(0), xs_pad.size(1), self.in_channel, xs_pad.size(2) // self.in_channel,
        ).transpose(1, 2)
        ...
        if etype.startswith("vgg"):
            if etype[-1] == "p":
                ...
                self.enc = torch.nn.ModuleList(
                    [
                        VGG2L(in_channel),
                        RNN(get_vgg2l_odim(idim, in_channel=in_channel), elayers, eunits, eprojs, dropout, typ=typ,),
                    ]
                )
