...
class CausalConv1d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, dilation=1, bias=True):
        ...
        self.conv = nn.Conv1d(in_channels, out_channels, kernel_size, padding=padding, dilation=dilation, bias=bias,)
...
class UpSampling(nn.Module):
    ...
    def __init__(self, upsampling_factor, bias=True):
        ...
        self.conv = nn.ConvTranspose2d(
            1, 1, kernel_size=(1, self.upsampling_factor), stride=(1, self.upsampling_factor), bias=self.bias,
        )
...
class WaveNet(nn.Module):
    ...
    def _residual_forward(
        self, x, h, dil_sigmoid, dil_tanh, aux_1x1_sigmoid, aux_1x1_tanh, skip_1x1, res_1x1,
    ):
        output_sigmoid = dil_sigmoid(x)
        output_tanh = dil_tanh(x)
        aux_output_sigmoid = aux_1x1_sigmoid(h)
        aux_output_tanh = aux_1x1_tanh(h)
        output = torch.sigmoid(output_sigmoid + aux_output_sigmoid) * torch.tanh(output_tanh + aux_output_tanh)
