...
class E2E(ASRInterface, torch.nn.Module):
    ...
    def encoder_add_arguments(parser):
        ...
        group.add_argument(
            "--eunits", "-u", type=int, action="append", help="Number of encoder hidden units",
        )
    ...
    def attention_add_arguments(parser):
        ...
        group.add_argument(
            "--adim", type=int, action="append", help="Number of attention transformation dimensions",
        )
        group.add_argument(
            "--awin", type=int, action="append", help="Window size for location2d attention",
        )
        group.add_argument(
            "--aheads", type=int, action="append", help="Number of heads for multi head attention",
        )
        group.add_argument(
            "--aconv-chans",
            type=int,
        ...
        group.add_argument(
            "--dropout-rate", type=float, action="append", help="Dropout rate for the encoder",
        )
        ...
        group.add_argument(
            "--han-dim", default=320, type=int, help="Number of attention transformation dimensions in HAN",
        )
        group.add_argument(
            "--han-win", default=5, type=int, help="Window size for location2d attention in HAN",
        )
        group.add_argument(
            "--han-heads", default=4, type=int, help="Number of heads for multi head attention in HAN",
        )
        group.add_argument(
            "--han-conv-chans",
            default=-1,
    ...
    def forward(self, xs_pad_list, ilens_list, ys_pad):
        ...
        if loss_data < CTC_LOSS_THRESHOLD and not math.isnan(loss_data):
            self.reporter.report(
                loss_ctc_data_list, loss_att_data, acc, cer_ctc_list, cer, wer, loss_data,
            )
        else:
    ...
    def recognize_batch(self, xs_list, recog_args, char_list, rnnlm=None):
        ...
        xs_list = [
            [to_device(self, to_torch_tensor(xx).float()) for xx in xs_list[idx]] for idx in range(self.num_encs)
        ]
        ...
        # 2. Decoder
        hlens_list = [
            torch.tensor(list(map(int, hlens_list[idx]))) for idx in range(self.num_encs)
        ]  # make sure hlens is tensor
        y = self.dec.recognize_beam_batch(
            hs_pad_list, hlens_list, lpz_list, recog_args, char_list, rnnlm, normalize_score=normalize_score,
        )
