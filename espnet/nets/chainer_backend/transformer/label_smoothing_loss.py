...
class LabelSmoothingLoss(chainer.Chain):
    ...
    def forward(self, ys_block, ys_pad):
        ...
        if not self.use_label_smoothing:
            ...
            broad_ignore_mask = self.xp.broadcast_to(ignore_mask[:, None], concat_logit_block.shape)
            pre_loss = ignore_mask * log_prob[self.xp.arange(batch * length), concat_t_block]
