    ...
    def __init__(self, channels, idim, dims, dropout=0.1, initialW=None, initial_bias=None):
        ...
        with self.init_scope():
            ...
            self.conv1 = L.Convolution2D(
                1, channels, 3, stride=2, pad=1, initialW=initialW(scale=stvd), initial_bias=initial_bias(scale=stvd),
            )
            n = channels * 3 * 3
            stvd = 1.0 / np.sqrt(n)
            ...
            self.out = L.Linear(idim, dims, initialW=initialW(scale=stvd), initial_bias=initial_bias(scale=stvd),)
