...
class Decoder(chainer.Chain):
    ...
    def __init__(
        ...
        with self.init_scope():
            ...
            self.rnn0 = (
                L.StatelessLSTM(dunits + eprojs, dunits) if dtype == "lstm" else L.StatelessGRU(dunits + eprojs, dunits)
            )
        ...
        if self.dtype == "lstm":
            ...
            if z_prev[0] is None:
                ...
                with chainer.backends.cuda.get_device_from_id(self._device_id):
                    z_prev[0] = chainer.Variable(xp.zeros((ey.shape[0], self.dunits), dtype=ey.dtype))
    ...
    def recognize_beam(self, h, lpz, recog_args, char_list, rnnlm=None):
        ...
        for i in six.moves.range(maxlen):
            ...
            logging.debug(
                "best hypo: " + "".join([char_list[int(x)] for x in hyps[0]["yseq"][1:]]).replace("<space>", " ")
            )
