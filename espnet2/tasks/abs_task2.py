...
class AbsTask(ABC):
    ...
    def main(
        ...
        # 3. Build epoch_scheduler: invoked at every epochs
        ...
        # 4. Build batch_scheduler: invoked after every updating
        ...
        # 6. Loads states from saved files
        reporter = Reporter()
        output_dir = Path(args.outpudir)
        ...
        (train_iter, eval_iter,
         plot_attention_iter) = cls.build_iterators(args)

        if args.stats_run:
            if reporter.get_epoch() == 0:
                tr_shape_files, ev_shape_files = cls.gather(
                    model=None,
            ...
            if args.train_batch_type != "const":
        ...
        if args.log_interval is None:
            log_interval = max(len(train_iter) // 20, 10)
        else:
    ...
    @classmethod
    def build_iterators(cls, args: argparse.Namespace):
        # 1. Build train-data-iterator
        ...
        if len(args.train_shape_file) == 0:
            train_key_file, _, _ = args.train_data_path_and_name_and_type[0]
            batch_type = "constant"
            train_shape_files = [train_key_file]
        else:
            batch_type = args.batch_type
            train_shape_files = args.train_shape_file
        ...
        if len(args.eval_shape_file) == 0:
            eval_key_file, _, _ = args.eval_data_path_and_name_and_type[0]
            eval_batch_type = "constant"
            eval_shape_files = [eval_key_file]
        else:
            if args.eval_batch_type is None:
                eval_batch_type = args.batch_type
            else:
                eval_batch_type = args.eval_batch_type
            eval_shape_files = args.eval_shape_file
            eval_key_file = args.eval_shape_file[0]
        ...
        eval_batch_sampler = build_batch_sampler(
            type=eval_batch_type,
            shape_files=eval_shape_files,
            max_lengths=args.max_length,
        ...
        return train_iter, eval_iter, plot_attention_iter
    ...
    def gather(cls,
               ...
               output_dir: Union[str, Path],
               model: Optional[AbsE2E],
               ) -> Tuple[List[str], List[str]]:
        output_dir = Path(output_dir)
        tr_datadir_writer = DatadirWriter(output_dir / 'gather_train')
        for ids, batch in train_iter:
            assert isinstance(batch, dict), type(batch)
            batch = to_device(batch, "cuda" if ngpu > 0 else "cpu")
            for k, v in batch.items():
                for id, v2 in zip(ids, v):
                    tr_datadir_writer[f'{k}_shape'][id] = ','.join(
                        map(str, v2.shape))
            if model is not None:
                model(**batch)

        ev_datadir_writer = DatadirWriter(output_dir / 'gather_eval')
        for ids, batch in eval_iter:
            assert isinstance(batch, dict), type(batch)
            batch = to_device(batch, "cuda" if ngpu > 0 else "cpu")
            for k, v in batch.items():
                for id, v2 in zip(ids, v):
                    ev_datadir_writer[f'{k}_shape'][id] = ','.join(
                        map(str, v2.shape))
            if model is not None:
                model(**batch)
        return list(tr_datadir_writer), list(ev_datadir_writer)
    ...
    def train(
        ...
        for iiter, ids, batch in enumerate(iterator, 1):
            assert isinstance(batch, dict), type(batch)
            batch = to_device(batch, "cuda" if ngpu > 0 else "cpu")
