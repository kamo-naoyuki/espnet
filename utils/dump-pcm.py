...
def get_parser():
    parser = argparse.ArgumentParser(
        description="dump PCM files from a WAV scp file", formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    ...
    parser.add_argument(
        ...
        type=strtobool,
        default=True,
        help="Truncating or zero padding if the output length " "is changed from the input by preprocessing",
    )
            ...
            if preprocessing is not None:
                ...
                if args.keep_length:
                    if len(out) > len(array):
                        out = numpy.pad(
                            out, [(0, len(out) - len(array))] + [(0, 0) for _ in range(out.ndim - 1)], mode="constant",
                        )
