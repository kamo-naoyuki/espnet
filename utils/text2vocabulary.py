...
def get_parser():
    parser = argparse.ArgumentParser(
        description="create a vocabulary file from text files", formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--output", "-o", default="", type=str, help="output a vocabulary file")
    parser.add_argument("--cutoff", "-c", default=0, type=int, help="cut-off frequency")
    parser.add_argument("--vocabsize", "-s", default=20000, type=int, help="vocabulary size")
...
if __name__ == "__main__":
    ...
    for fn in args.text_files:
        fd = (
            ...
            if fn != "-"
            else codecs.getreader("utf-8")(sys.stdin if is_python2 else sys.stdin.buffer)
        )
