...
def get_parser():
    ...
    parser.add_argument(
        "--input-scps", type=str, nargs="*", action="append", default=[], help="Json files for the inputs",
    )
    parser.add_argument(
        "--output-scps", type=str, nargs="*", action="append", default=[], help="Json files for the outputs",
    )
    parser.add_argument(
        "--scps", type=str, nargs="+", default=[], help="The json files except for the input and outputs",
    )
    ...
    parser.add_argument(
        ...
        type=strtobool,
        default=False,
        help="Allow one column in input scp files. " "In this case, the value will be empty string.",
    )
    parser.add_argument(
        "--out", "-O", type=str, help="The output filename. " "If omitted, then output to sys.stdout",
    )
    return parser
...
if __name__ == "__main__":
    ...
    for lis_list, key_scps_list in [
        ...
        for key_scps in key_scps_list:
            ...
            for key_scp in key_scps:
                ...
                for item in lis:
                    if key == item[0]:
                        raise RuntimeError('The key "{}" is duplicated: {} {}'.format(key, item[3], key_scp))

                lis.append((key, scp, type_func, key_scp, type_func_str))
            lis_list.append(lis)

    # Open  scp files
    input_fscps = [[open(i[1], "r", encoding="utf-8") for i in il] for il in input_infos]
    output_fscps = [[open(i[1], "r", encoding="utf-8") for i in il] for il in output_infos]
    ...
    while True:
                    ...
                    if line == "" or first == "":
                        if line != first:
                            ...
                            raise RuntimeError(
                                ...
                                'between: "{}" and "{}"'.format(concat[0][1], concat[count][1])
                            )
                        ...
                        raise RuntimeError(
                            ...
                            'between "{}" and "{}":\n>>> {}\n>>> {}'.format(
                                nutt, concat[0][1], concat[count][1], first.rstrip(), line.rstrip(),
                            )
                        )
                    count += 1
        ...
        entry = json.dumps(entry, indent=4, ensure_ascii=False, sort_keys=True, separators=(",", ": "))
        # Add indent
        indent = "    " * 2
