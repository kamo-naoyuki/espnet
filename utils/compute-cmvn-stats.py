...
def get_parser():
    ...
    parser.add_argument("rspecifier", type=str, help="Read specifier for feats. e.g. ark:some.ark")
    parser.add_argument("wspecifier_or_wxfilename", type=str, help="Write specifier. e.g. ark:some.ark")
    return parser
...
def main():
    ...
    if is_wspecifier:
        ...
        if args.out_filetype == "npy":
            logging.warning("--out-filetype npy is allowed only for " "Global CMVN mode, changing to hdf5")
        ...
        if args.out_filetype == "hdf5":
            logging.warning("--out-filetype hdf5 is not allowed for " "Global CMVN mode, changing to npy")
    ...
    idx = 0
    for idx, (utt, matrix) in enumerate(file_reader_helper(args.rspecifier, args.in_filetype), 1):
    ...
    if is_wspecifier:
        with file_writer_helper(args.wspecifier_or_wxfilename, filetype=args.out_filetype) as writer:
