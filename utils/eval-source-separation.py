...
def eval_STOI(ref, y, fs, extended=False, compute_permutation=True):
    ...
    if ref.shape != y.shape:
        raise ValueError("ref and y should have the same shape: {} != {}".format(ref.shape, y.shape))
    if ref.ndim != 3:
    ...
    values = [
        [
            sum(stoi(ref[i, :, ch], y[j, :, ch], fs, extended) for ch in range(n_mic)) / n_mic
            for i, j in enumerate(indices)
        ]
        for indices in index_list
    ]

    best_pairs = sorted([(v, i) for v, i in zip(values, index_list)], key=lambda x: sum(x[0]))[-1]
...
def eval_PESQ(ref, enh, fs, compute_permutation: bool = True, wideband: bool = True):
    ...
    if ref.shape != enh.shape:
        raise ValueError("ref and enh should have the same shape: {} != {}".format(ref.shape, enh.shape))
    if ref.ndim != 3:
        raise ValueError("Input must have 3 dims: {}".format_map(ref.ndim))
        ...
        for indices in index_list:
            ...
            for i, j in enumerate(indices):
                ...
                for imic in range(n_mic):
                    ...
                    with subprocess.Popen(commands, stdout=subprocess.DEVNULL, cwd=d) as p:
                        _, _ = p.communicate()
...
def get_parser():
    ...
    parser.add_argument(
        "--ref", dest="reffiles", nargs="+", type=str, required=True, help="WAV file lists for reference",
    )
    parser.add_argument(
        "--enh", dest="enhfiles", nargs="+", type=str, required=True, help="WAV files lists for enhanced",
    )
    parser.add_argument("--outdir", type=str, required=True)
    parser.add_argument(
        "--keylist",
        type=str,
        help="Specify the target samples. By default, " "using all keys in the first reference file",
    )
    parser.add_argument(
        "--evaltypes",
        type=str,
    ...
    parser.add_argument(
        ...
        type=strtobool,
        default=True,
        help="Use bss_eval_images or bss_eval_sources. " "For more detail, see museval source codes.",
    )
    parser.add_argument(
        "--bss-eval-version", type=str, default="v3", choices=["v3", "v4"], help="Specify bss-eval-version: v3 or v4",
    )
    return parser
...
def main():
    ...
    if len(args.reffiles) != len(args.enhfiles):
        raise RuntimeError(
            ...
            "from the enh files: {} != {}".format(len(args.reffiles), len(args.enhfiles))
        )
    ...
    for key in keylist:
        ...
        ref_signals = [
            np.pad(s, [(0, ml - len(s)), (0, 0)], mode="constant") if len(s) < ml else s for s in ref_signals
        ]

        enh_signals = [
            np.pad(s, [(0, ml - len(s)), (0, 0)], mode="constant") if len(s) < ml else s for s in enh_signals
        ]
        ...
        for evaltype in args.evaltypes:
            if evaltype == "SDR":
                ...
                stoi, perm = eval_STOI(
                    ref_signals, enh_signals, rate, extended=False, compute_permutation=args.permutation,
                )
                writers["STOI"].write("{} {}\n".format(key, " ".join(map(str, stoi))))

            elif evaltype == "ESTOI":
                estoi, perm = eval_STOI(
                    ref_signals, enh_signals, rate, extended=True, compute_permutation=args.permutation,
                )
                writers["ESTOI"].write("{} {}\n".format(key, " ".join(map(str, estoi))))

            elif evaltype == "PESQ":
                pesq, perm = eval_PESQ(ref_signals, enh_signals, rate, compute_permutation=args.permutation)
